...
* python_can. [python-can](https://python-can.readthedocs.io/en/stable/installation.html)
...
This library provides a simple interface to the ZDT drive, allowing you to send commands and receive responses easily.
...
* Support for all checksums supported by the board through `lib_bytes.ChecksumABC` and its subclasses. These can be accessed with `lib_bytes.Checksums` enum as well for easier use. Checksums are:
    ...
    * `lib_bytes.Checksums.CSXOR`: XOR checksum (XOR of all bytes) 
    * `lib_bytes.Checksums.CSCRC8`: CRC8 checksum
...
* As it is now, the library is only made to drive a single ZDT drive (only 1 python-can Bus object can be attached to the CAN USB interface).
* Also, since each commands expect a return packet from the controller with thus the same ID, and otherwise returns a fail upon receiving a packet with unpexpected id, it is not possible to send several commands at once or drive multiple ZDT drives at once.

Using a static Bus object alongside filters might provide a viable approach to these issues.
...
I would be happy for any contributions to this project. Please also feel free to fork it and use it as you see fit.
