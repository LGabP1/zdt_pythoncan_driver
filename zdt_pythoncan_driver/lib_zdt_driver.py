...
"""
from .lib_interface import CANInterface
...
class ZDTSystemStatusParameters:
    ...
    Parameters:
        received_bytes (int): Number of bytes in the command 
        num_config_params (int): Number of configuration parameters in the command
        bus_voltage (int): In mV.
        bus_phase_current (int): In mA.
        calibrated_encoder_value (int): 0-65535 being a full rotation.
        motor_target_position (int): 0-65535 being a full rotation.
        motor_real_time_speed (int): In RPM.
        motor_real_time_position (int): 0-65535 being a full rotation.
        motor_position_error (int): 0-32767=0x7FFF being a full turn.
...
class ZDTHomingParameters:
    ...
    Parameters:
        ...
        - direction (Literal["Clockwise", "Counter-clockwise"]): direction "Clockwise" or "Counter-clockwise".
...
class ZDTEmmV50Handler:
    ...
    def __init__(self, interface: str, channel: str, bitrate: int, mot_id: int, checksum: ChecksumABC = Checksums.CS0x6B, send_timeout: float = 1.0, recv_timeout: float = 1.0, logger: Optional[Logger] = None):
        ...
        Args: 
            ...
            """
        self.can_interface = CANInterface(interface, channel, bitrate, logger=logger) # can.Bus object wrapper
        self.mot_id = mot_id
    ...
    def cmd_trigger_encoder_CAL(self) -> ZDTReturnCode:
        ...
        self.can_interface.send(self.mot_id, payload, self.checksum) # ID inserted in payload, checksum appended by interface

        recv_data, _id, status = self.can_interface.recv(self.recv_timeout, self.checksum)
        if not status:
            return (ZDTReturnCode.NONE)
        if _id != self.mot_id:
            return (ZDTReturnCode.NONE) # invalid device id
        ...
        if recv_data[0] == 0x06 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
    ...
    def cmd_reset_position_to_0(self) -> ZDTReturnCode:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xee: # manual check if error
            return ZDTReturnCode.ERROR
        if recv_data[0] == 0x0A and recv_data[1] == 0x02 and self._check_checksum(recv_data): # Ok
            return ZDTReturnCode.OK
        return ZDTReturnCode.NONE
    ...
    def cmd_release_stall_protection(self) -> ZDTReturnCode:
        ...
        if recv_data[0] == 0x0E and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0x0E and recv_data[1] == 0x02 and self._check_checksum(recv_data): # Ok
            return ZDTReturnCode.OK
        return ZDTReturnCode.NONE
    ...
    def cmd_factory_reset(self) -> ZDTReturnCode:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return ZDTReturnCode.ERROR
        if recv_data[0] == 0x0F and recv_data[1] == 0x02 and self._check_checksum(recv_data): # Ok
            return ZDTReturnCode.OK
        return ZDTReturnCode.NONE
    ...
    def cmd_read_firmware_and_hardware_version(self) -> tuple[ZDTReturnCode, int, int]:
        ...
        if _id != self.mot_id:
            return (ZDTReturnCode.NONE, 0, 0) # invalid device id
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0, 0)
        if recv_data[0] == 0x1F and self._check_checksum(recv_data): # Ok
    ...
    def cmd_read_phase_resistance_and_inductance(self) -> tuple[ZDTReturnCode, int, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0, 0)
        if recv_data[0] == 0x20 and self._check_checksum(recv_data): # Ok
            resistance_value = int.from_bytes(recv_data[1:3], byteorder='big', signed=True)
            phase_inductance = int.from_bytes(recv_data[3:5], byteorder='big', signed=True)
    ...
    def cmd_read_position_PID_parameters(self) -> tuple[ZDTReturnCode, int, int, int]:
        ...
        if _id != self.mot_id:
            return (ZDTReturnCode.NONE, 0, 0, 0) # invalid device id
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0, 0, 0)
        if recv_data[0] == 0x21 and self._check_checksum(recv_data): # Ok
            kp = int.from_bytes(recv_data[1:5], byteorder='big', signed=True)
            ki = int.from_bytes(recv_data[5:9], byteorder='big', signed=True)
            kd = int.from_bytes(recv_data[9:13], byteorder='big', signed=True)
            return (ZDTReturnCode.OK, kp, ki, kd)
        return (ZDTReturnCode.NONE, 0, 0, 0)

    def cmd_read_origin_homing_parameters(self) -> tuple[ZDTReturnCode, ZDTHomingParameters | None]: # TODO: to test, with write (could not find any read change)
        ...
        if _id != self.mot_id:
            return (ZDTReturnCode.NONE, None) # invalid device id
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, None)
        if recv_data[0] == 0x21 and self._check_checksum(recv_data): # Ok
            mode: Literal["Nearest", "Directional Nearest", "Collision", "Limit Switch"] = ("Nearest", "Directional Nearest", "Collision", "Limit Switch")[recv_data[1]]     
            direction = "Clockwise" if recv_data[2] == 0x00 else "Counter-Clockwise" # 
            speed = int.from_bytes(recv_data[3:5], byteorder='big', signed=True)
            timeout = int.from_bytes(recv_data[5:9], byteorder='big', signed=True)
            detection_speed = int.from_bytes(recv_data[9:11], byteorder='big', signed=True)
            detection_current = int.from_bytes(recv_data[11:13], byteorder='big', signed=True)
            detection_time =  int.from_bytes(recv_data[13:15], byteorder='big', signed=True)
    ...
    def cmd_read_bus_voltage(self) -> tuple[ZDTReturnCode, int]:
        ...
        if _id != self.mot_id:
            return (ZDTReturnCode.NONE, 0) # invalid device id
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x24 and self._check_checksum(recv_data): # Ok
            voltage = int.from_bytes(recv_data[1:3], byteorder='big', signed=True)
    ...
    def cmd_read_phase_current(self) -> tuple[ZDTReturnCode, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x27 and self._check_checksum(recv_data): # Ok
            current = int.from_bytes(recv_data[1:3], byteorder='big', signed=True)
    ...
    def cmd_read_CAL_encoder_value(self) -> tuple[ZDTReturnCode, int]:    
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x31 and self._check_checksum(recv_data): # Ok
            value = int.from_bytes(recv_data[1:3], byteorder='big', signed=True)
    ...
    def cmd_read_input_pulse_count(self) -> tuple[ZDTReturnCode, int]: 
        ...
        Returns tuple[return_code: ZDTReturnCode, pulse_count: int]:
            return_code (ZDTReturnCode):
                ...
                - ZDTReturnCode.ERROR if error code 00 EE xx
                - ZDTReturnCode.NONE if no return received.
            pulse_count (int in [-65535, 65535]): (Signed) input pulse count. Will be 0 if error."""
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x32 and self._check_checksum(recv_data): # Ok
            sign = +1 if recv_data[1] == 0x00 else -1 # get sign
            pulse_count = sign*int.from_bytes(recv_data[2:4], byteorder='big', signed=True)
    ...
    def cmd_read_closed_loop_target_position(self) -> tuple[ZDTReturnCode, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x33 and self._check_checksum(recv_data): # Ok
            sign = +1 if recv_data[1] == 0x00 else -1 # get sign
            target_pos = sign*int.from_bytes(recv_data[2:6], byteorder='big', signed=True)
    ...
    def cmd_read_open_loop_target_position(self) -> tuple[ZDTReturnCode, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x34 and self._check_checksum(recv_data): # Ok
            sign = +1 if recv_data[1] == 0x00 else -1 # get sign
            target_pos = sign*int.from_bytes(recv_data[2:6], byteorder='big', signed=True)
            return (ZDTReturnCode.OK, target_pos)
        return (ZDTReturnCode.NONE, 0)
    ...
    def cmd_read_current_speed(self) -> tuple[ZDTReturnCode, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x35 and self._check_checksum(recv_data): # Ok
            sign = +1 if recv_data[1] == 0x00 else -1 # get sign
            speed = sign*int.from_bytes(recv_data[2:4], byteorder='big', signed=True)
    ...
    def cmd_read_current_position(self) -> tuple[ZDTReturnCode, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x36 and self._check_checksum(recv_data): # Ok
            sign = +1 if recv_data[1] == 0x00 else -1 # get sign
            position = sign*int.from_bytes(recv_data[2:6], byteorder='big', signed=True)
    ...
    def cmd_read_current_position_error(self) -> tuple[ZDTReturnCode, int]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, 0)
        if recv_data[0] == 0x37 and self._check_checksum(recv_data): # Ok
            sign = +1 if recv_data[1] == 0x00 else -1 # get sign
            position_error = sign*int.from_bytes(recv_data[2:6], byteorder='big', signed=True)
    ...
    def cmd_read_motor_status_flags(self) -> tuple[ZDTReturnCode, ZDTMotorStatusFlags | None]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, None)
        if recv_data[0] == 0x3A and self._check_checksum(recv_data): # Ok
    ...
    def cmd_read_homing_status_flags(self) -> tuple[ZDTReturnCode, ZDTHomingStatusFlags | None]:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, None)
        if recv_data[0] == 0x3B and self._check_checksum(recv_data): # Ok
            flag_byte = recv_data[1]
    ...
    def cmd_read_drive_configuration_parameters(self) -> tuple[ZDTReturnCode, ZDTDriveConfigurationParameters | None]: # TODO: to test
        ...
        Parameter descriptions: #TODO: TODO & copy these descriptions to ZDTDriveConfigurationParameters docstring + comment 0x48
            ...
            dir_pin_effective_direction (int in [0,255]): 0 is Clockwise, 1 is Counter-clockwise
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, None)
        if recv_data[0] == 0x42 and self._check_checksum(recv_data): # Ok ?
            ...
            out_params = ZDTDriveConfigurationParameters( #Ok !
                ...
                open_loop_mode_operating_current = int.from_bytes(recv_data[11:13], byteorder='big', signed=True),
                closed_loop_mode_maximum_stall_current = int.from_bytes(recv_data[13:15], byteorder='big', signed=True),
                closed_loop_mode_maximum_output_voltage = int.from_bytes(recv_data[15:17], byteorder='big', signed=True),
                ...
                stall_protection_speed_threshold = int.from_bytes(recv_data[23:25], byteorder='big', signed=True),
                stall_protection_current_threshold = int.from_bytes(recv_data[25:27], byteorder='big', signed=True),
                stall_protection_detection_time_threshold = int.from_bytes(recv_data[27:29], byteorder='big', signed=True),
                position_arrival_window = int.from_bytes(recv_data[29:31], byteorder='big', signed=True)
            )
            return (ZDTReturnCode.OK, out_params)
        return (ZDTReturnCode.NONE, None)
    
    def cmd_read_system_status_parameters(self) -> tuple[ZDTReturnCode, ZDTSystemStatusParameters | None]: #TODO: test !
        ...
        Parameter descriptions:  TODO: document params + update ZDTSystemStatusParameters docstring
            received_bytes (int in [0,255]): Number of bytes in the command 
            num_config_params (int in [0,255]): Number of configuration parameters in the command 
            bus_voltage (in in [0,65535]): In mV. #TODO: test !
            bus_phase_current (in in [0,65535]): In mA. #TODO: test !
            calibrated_encoder_value (in in [0,65535]): 0-65535 being a full rotation. #TODO: test !
            motor_target_position (in in [-4294967295,4294967295]): 0-65535 being a full rotation. #TODO: test !
            motor_real_time_speed (in in [-65535,65535]): In RPM. #TODO: test !
            motor_real_time_position (in in [-4294967295,4294967295]): 0-65535 being a full rotation. #TODO: test !
            motor_position_error (in in [-4294967295,4294967295]): 0-32767=0x7FFF being a full turn. #TODO: test !
            flag_encoder_ready_status (bool): Whether encoder ready. #TODO: test !
            flag_calibration_ready_status (bool): Whether calibration ready. #TODO: test !
            flag_homing_in_progress (bool): Whether Homing in progress. #TODO: test !
            flag_homing_failure (bool): Whether Homing failure. #TODO: test !
            flag_enable_motor_status (bool): Whether motor enabled. #TODO: test !
            flag_motor_in_position_flag (bool): Whether motor is in position. #TODO: test !
            flag_motor_stall (bool): Whether motor is in stall mode. #TODO: test !
            flag_stall_protection (bool): Whether motor is stall protected. #TODO: test !"""
        payload = bytearray([0x43, 0x7A])
        self.can_interface.send(self.mot_id, payload, self.checksum) # ID inserted in payload, checksum appended by interface

        recv_data, _id, status = self.can_interface.recv(self.recv_timeout, self.checksum)
        if not status:
            return (ZDTReturnCode.NONE, False)
        if _id != self.mot_id:
            return (ZDTReturnCode.NONE, False) # invalid device id
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR, False)
        if recv_data[0] == 0x43 and self._check_checksum(recv_data): # Ok
            if len(recv_data) + 1 != recv_data[1]: # verify number of bytes
                return (ZDTReturnCode.ERROR, None)
            # Ok !
            mot_target_pos_sign = -1 if recv_data[9] == 0x01 else +1
            mot_target_pos = mot_target_pos_sign*int.from_bytes(recv_data[10:14], byteorder='big', signed=True),
            mot_rt_speed_sign = -1 if recv_data[14] == 0x01 else +1
            mot_rt_speed = mot_rt_speed_sign*int.from_bytes(recv_data[15:17], byteorder='big', signed=True),
            mot_rt_pos_sign = -1 if recv_data[17] == 0x01 else +1
            mot_rt_pos = mot_rt_pos_sign*int.from_bytes(recv_data[18:24], byteorder='big', signed=True),
            mot_pos_error_sign = -1 if recv_data[24] == 0x01 else +1
            mot_pos_error = mot_pos_error_sign*int.from_bytes(recv_data[25:29], byteorder='big', signed=True),
            ready_status_flag_byte = recv_data[29]
            motor_status_flag_byte = recv_data[30]

            out_params = ZDTSystemStatusParameters(
                received_bytes=recv_data[1],
                num_config_params=recv_data[2],
                bus_voltage=int.from_bytes(recv_data[3:5], byteorder='big', signed=True),
                bus_phase_current=int.from_bytes(recv_data[5:7], byteorder='big', signed=True),
                calibrated_encoder_value=int.from_bytes(recv_data[7:9], byteorder='big', signed=True),
    ...
    def cmd_edit_open_loop_operating_current(self, do_store: bool, ol_mode_current: int) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x44 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)

    def cmd_switch_open_loop_closed_loop(self, do_store: bool, new_mode: Literal["OpenLoop", "CloseLoop"]) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x46 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    ...
    def cmd_edit_drive_config_parameters(self, do_store: bool, drive_config_params: ZDTDriveConfigurationParameters) -> ZDTReturnCode:
        ...
        payload = bytearray([0x48, 0xD1, storage_flag])
        payload.extend(bytearray([drive_config_params.mot_type]))
        payload.extend(bytearray([drive_config_params.pulse_port_control_mode]))
        payload.extend(bytearray([drive_config_params.communication_port_multiplexing_mode]))
        payload.extend(bytearray([drive_config_params.en_pin_active_level]))
        payload.extend(bytearray([drive_config_params.dir_pin_effective_direction]))
        payload.extend(bytearray([drive_config_params.microstep]))
        payload.extend(bytearray([microstep_enabled_byte]))
        payload.extend(bytearray([auto_screen_off_enabled_byte]))
        payload.extend(drive_config_params.open_loop_mode_operating_current.to_bytes(2, "big", signed=False))
        payload.extend(drive_config_params.closed_loop_mode_maximum_stall_current.to_bytes(2, "big", signed=False))
        payload.extend(drive_config_params.closed_loop_mode_maximum_output_voltage.to_bytes(2, "big", signed=False))
        payload.extend(bytearray([drive_config_params.serial_baud_rate]))
        payload.extend(bytearray([drive_config_params.CAN_communication_rate]))
        payload.extend(bytearray([0x01])) # Deprecated param
        payload.extend(bytearray([drive_config_params.communication_checksum_method]))
        payload.extend(bytearray([drive_config_params.control_command_response]))
        payload.extend(bytearray([stall_protection_enabled_byte]))
        payload.extend(drive_config_params.stall_protection_speed_threshold.to_bytes(2, "big", signed=False))
        payload.extend(drive_config_params.stall_protection_current_threshold.to_bytes(2, "big", signed=False))
        payload.extend(drive_config_params.stall_protection_detection_time_threshold.to_bytes(2, "big", signed=False))
        payload.extend(drive_config_params.position_arrival_window.to_bytes(2, "big", signed=False))
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x48 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)

    def cmd_edit_position_loop_PID_parameters(self, do_store: bool, kp: int, ki: int, kd: int) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x4A and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    
    def cmd_set_homing_parameters(self, do_store: bool, homing_params: ZDTHomingParameters) -> ZDTReturnCode: #TODO: test !
        ...
        match homing_params.direction:
            ...
            case _:
                raise AssertionError(f'{homing_params.direction=} should have been either "Clockwise" or "Counter-clockwise"')
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x4C and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)

    def cmd_allow_divide10_on_com_speed_commands(self, do_store: bool, do_divide10: bool) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x4F and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    
    def cmd_edit_any_Microstep(self, do_store: bool, microstep_value: int) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x84 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)

    def cmd_set_0_position_homing(self, do_store: bool) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x93 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)

    def cmd_trigger_homing(self, homing_mode: Literal["Nearest", "Directional Nearest", "Collision", "Limit Switch"], m_m_sync_flag: bool = False) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x9A and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0x9A and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    
    def cmd_interrupt_homing(self) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x9C and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0x9C and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    

    def cmd_edit_ID_address(self, do_store: bool, target_id: int) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xEE: # manual check if error
            return (ZDTReturnCode.ERROR)
        if recv_data[0] == 0x4B and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    ...
    def cmd_motor_enable_control(self, enable: bool = True, m_m_sync_flag: bool = False) -> ZDTReturnCode:
        ...
        m_m_sync_flag_byte = 0x01 if m_m_sync_flag else 0x00
        data = bytearray([0xF3, 0xAB, enable_status, m_m_sync_flag_byte])
        self.can_interface.send(self.mot_id, data, self.checksum) # ID inserted in payload, checksum appended by interface
        recv_data, _id, status = self.can_interface.recv(self.recv_timeout, self.checksum)
        if not status:
            return ZDTReturnCode.NONE
        if _id != self.mot_id:
        ...
        if recv_data[0] == 0x00 and recv_data[1] == 0xee: # manual check if error
            return ZDTReturnCode.ERROR
        if self._check_checksum(recv_data):
            return ZDTReturnCode.OK
        return ZDTReturnCode.NONE
        
    def cmd_speed_mode_control(self, speed: int, acceleration_level: int, m_m_sync_flag: bool = False) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0xF6 and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0xF6 and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return ZDTReturnCode.OK
        return ZDTReturnCode.NONE
        
    def cmd_auto_run_on_power_on(self, store_clear_flag: int, speed: int, acceleration_level: int, do_en_pin_control: bool) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0xFD and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0xFD and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return ZDTReturnCode.OK
        return ZDTReturnCode.NONE
    ...
    def cmd_position_mode_control(self, pulse_count: int, speed: int, acceleration_level: int, is_relative: bool = True, m_m_sync_flag: bool = False) -> ZDTReturnCode:
        ...
        # TODO: wait for long enough to receive position reached notice ?
        """
    ...
    def cmd_immediate_stop(self, m_m_sync_flag: bool = False) -> ZDTReturnCode: #TODO: test !
        ...
        if recv_data[0] == 0xFE and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0xFE and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)
    

    def cmd_M_M_synchronized_motion(self, do_broadcast: bool) -> ZDTReturnCode: #TODO: test !
        ...
        if do_broadcast:       
            self.can_interface.send(0x00, payload, self.checksum) # Force broadcast # ID inserted in payload, checksum appended by interface
        else:
            self.can_interface.send(self.mot_id, payload, self.checksum) # ID inserted in payload, checksum appended by interface
        ...
        if recv_data[0] == 0xFF and recv_data[1] == 0xE2: # manual check if conditions not met
            return ZDTReturnCode.CONDITIONS_NOT_MET
        if recv_data[0] == 0xFF and recv_data[1] == 0x02 and self._check_checksum(recv_data): # OK
            return (ZDTReturnCode.OK)
        return (ZDTReturnCode.NONE)